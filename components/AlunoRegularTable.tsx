...
const AlunoRegularTable: React.FC<AlunoRegularTableProps> = ({ data, onUpdate, onDelete, filters }) => {
  ...
  const exportToExcel = useCallback(() => {
    const exportData = data.map(aluno => ({
        ...aluno,
        'DURAÇÃO': calculateDuration(aluno)
    }));
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {paginatedData.map(a => (
              <tr key={a.id} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                ...
                <td className="px-3 py-4 whitespace-normal text-xs text-gray-500 dark:text-gray-300">{a.orientador}</td>
                <td className="px-3 py-4 whitespace-normal text-xs text-gray-500 dark:text-gray-300">{a.coOrientador || '-'}</td>
                <td className="px-3 py-4 whitespace-nowrap text-xs text-gray-500 dark:text-gray-300">{calculateDuration(a)}</td>
                <td className="px-3 py-4 whitespace-nowrap text-xs text-gray-500 dark:text-gray-300">{a.proficiencia || '-'}</td>
                <td className="px-3 py-4 whitespace-nowrap text-xs text-gray-500 dark:text-gray-300">{a.qualificacao || '-'}</td>

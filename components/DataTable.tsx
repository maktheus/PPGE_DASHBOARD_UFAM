...
const DataTable: React.FC<DataTableProps> = ({ data, onUpdate, onDelete }) => {
  ...
  const headers: { key: SortKey; label: string }[] = [
    ...
    { key: 'anoIngresso', label: 'Ano Ingresso' },
    { key: 'anoDefesa', label: 'Ano Defesa' },
    ...
    <div className="w-full">
      ...
      <div className="flex items-center justify-between py-3">
        <div className="text-sm text-gray-700 dark:text-gray-400">
            Mostrando {Math.min(paginatedData.length, (currentPage -1) * itemsPerPage + 1)} a {Math.min(currentPage * itemsPerPage, data.length)} de {data.length} resultados
        </div>
